...
plt.bar(range(NB_IAS), victory, tick_label = ["naive", "RC", "ERC", "CL", "GB"])
plt.ion()
plt.show()
for i in tqdm(range(NB_GAMES)):
    ...
    plt.ion()
    plt.show()
print(f"Naif: {victory[0]} - RC: {victory[1]} - ERC: {victory[2]} - CL: {victory[3]} - GB: {victory[4]}")
plt.clf()
plt.bar(range(NB_IAS), victory, tick_label = ["naive", "RC", "ERC", "CL", "GB"])
plt.pause(0.2)
plt.show()
